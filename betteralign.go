...
func run(pass *analysis.Pass) (interface{}, error) {
	...
	applyOps := make(map[string][]byte)

	inspect.Preorder(nodeFilter, func(node ast.Node) {
		if !testFiles && strings.HasSuffix(pass.Fset.File(node.Pos()).Name(), "_test.go") {
			return
		}
		...
		if tv, ok := pass.TypesInfo.Types[s]; ok {
			betteralign(pass, s, tv.Type.(*types.Struct), dec, dFile, applyOps)
		}
	})
...
func betteralign(pass *analysis.Pass, aNode *ast.StructType, typ *types.Struct, dec *decorator.Decorator,
	dFile *dst.File, fixOps map[string][]byte,
) {
	...
	fn := pass.Fset.File(aNode.Pos())
	fixOps[fn.Name()] = buf.Bytes()
}
